...
<div class="Menu">
		<div class="MenuBox">
			...
			<form action="/servletBM/book/search.do">
				<span>작가별 검색 : </span><input type="text" name="author"/><br>
				<span>인기순 정렬 : </span><input type="radio" name="orderBy" value="B_RENT_CNT"/>
				<span>최근 도서 순 정렬 : </span><input type="radio" name="orderby" value="B_REGDATE"/><br>
				<button>검색</button>
			</form>
		...
		<div class="recentBookBox">
			...
			<c:forEach items="${data.booklist}" var="b" varStatus="status">
				...
				<div class="productItem">
					...
					<div class="bookTitle">${b.b_title}<br>작가:${b.b_author}</div>
					<div class="bookUse" id="bookUse">
						<div><a href="/servletBM/book/detail.do?b_bno=${b.b_bno}">상세보기</a></div>
					</div>
				</div>
				</c:if>
				<c:if test="${status.count%4 == 0 }">
